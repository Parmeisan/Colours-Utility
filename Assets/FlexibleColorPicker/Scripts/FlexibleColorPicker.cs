...
public class FlexibleColorPicker : MonoBehaviour {
    ...
    */
    ...
    protected Transform preset_list;
    public void Start() {
        preset_list = transform.Find("Presets");
    }
    public void ToggleFull() { ToggleFull(!isFull); }
    public void ToggleFull(bool full)
    {
        foreach (Transform child in transform)
        {
            if (child.name != "ColorPreview")
            {
                child.gameObject.SetActive(full);
            }
        }
        GetComponent<Image>().enabled = full;
        isFull = full;
    }

    public void SelectPreset(int n)
    {
        // To the nth child of "Presets"
        var child = preset_list.GetChild(n);
